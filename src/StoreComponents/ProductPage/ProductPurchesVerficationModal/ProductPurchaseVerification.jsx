...
// EMERGENCY FIX: Lazy load the massive country data to prevent iOS Safari memory overload
...
const loadCountryData = async () => {
  if (!CountryData) {
    try {
      ...
      StateData = { getStatesOfCountry: () => [] };
      CityData = { getCitiesOfState: () => [] };
    }
  }
  return { Country: CountryData, State: StateData, City: CityData };
};
...
const ProductPurchaseVerificationModal = ({
  ...
  // Load country data only when modal is opened (lazy loading for iOS Safari)
  const initializeCountryData = async () => {
    if (countryDataLoaded) return;

    console.log("🌍 Loading country data...");
    const { Country, State, City } = await loadCountryData();

    const allCountries = Country.getAllCountries();
    setShippingCountries(allCountries);
    setBillingCountries(allCountries);
  ...
  useEffect(() => {
    ...
    if (data) {
      ...
      if (parsed.shipping_state) {
        ...
        setShippingCities(
          City.getCitiesOfState(parsed.shipping_country, parsed.shipping_state)
        );
      }
      ...
      if (parsed.billing_state) {
        ...
        setBillingCities(
          City.getCitiesOfState(parsed.billing_country, parsed.billing_state)
        );
      }
    }
  ...
  const showModal = async () => {
    ...
    // Only load country data when user actually opens the modal
        ...
        if (status === "OK") {
          ...
          if (element.status === "OK") {
            ...
            localStorage.setItem(
              "locationDetails",
              JSON.stringify(newLocationDetails)
            );
            ...
            notification.success({
              message: "Success",
              description:
                "Your address details saved locally. Please proceed to payment.",
              placement: "topRight",
            });
          } else {
            setLocationDetails(null);
      ...
      if (allValues.shipping_state) {
        ...
        setBillingCities(
          City.getCitiesOfState(
            allValues.shipping_country,
            allValues.shipping_state
          )
        );
      } else {
        setSelectedBillingState("");
  ...
  const formatAddressPreview = (data) => {
    ...
    return (
      <div>
        <b>
          {data.shipping_firstName?.slice(0, 12)}{" "}
          {data.shipping_lastName?.slice(0, 12)}
        </b>
        <br />
        {data.shipping_email && (
          <>
            Email: {data.shipping_email.slice(0, 12)}
            <br />
          </>
        )}
        {data.shipping_mobile && (
          <>Mobile: {data.shipping_mobile.slice(0, 12)}</>
        )}
      </div>
    );
  };
    ...
    >
      {!savedData ? (
        <Button
          ...
          onClick={showModal}
          size="large"
          style={{ margin: "0px", backgroundColor: "rgb(0, 0, 0)" }}
        >
          Process Order
        </Button>
      ) : (
        <>
          <div
            ...
            className="EditCssNew"
          >
            <br />
            <Title level={5}>Your Address</Title>
            <div>{formatAddressPreview(savedData)}</div>
      ...
      >
        ...
        >
          ...
          >
            ...
            <Form.Item label="Company Name (Optional)" name="shipping_company">
              <Input placeholder="Company Name" />
            </Form.Item>

            <Form.Item label="GSTIN (Optional)" name="shipping_gstin">
          ...
          {!sameAsBilling && (
            <>
              ...
              >
                ...
                <Form.Item label="GSTIN (Optional)" name="billing_gstin">
                  <Input placeholder="GSTIN" />
                </Form.Item>
